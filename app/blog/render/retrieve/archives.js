...
module.exports = {
  getAllEntries: function (req, callback) {
  ...
  getEntriesByDateRange: function (req, fromDate, toDate, callback) {
    Entries.getAll(req.blog.id, function (allEntries) {
      var years = {};

      for (var x in allEntries) {
        var entry = allEntries[x];
        var entryDate = moment.utc(entry.dateStamp).tz(req.blog.timeZone);

        if (fromDate && entryDate.isBefore(fromDate)) continue;
        if (toDate && entryDate.isAfter(toDate)) continue;

        var year = entryDate.format("YYYY");
        var month = entryDate.format("MMMM");
